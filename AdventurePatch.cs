...
//        }
//    }
//}   OLd version without delayed spawning in custom encounters
...
namespace AdventurePatch
{
    ...
    class SpawnForcePatch
    {
        ...
        private static MonoBehaviour GetCoroutineRunner()
        {
            ...
            {
                GameObject go = GameObject.Find("AdventurePatchCoroutineRunner");
                if (go == null)
                {
                    go = new GameObject("AdventurePatchCoroutineRunner");
                    UnityEngine.Object.DontDestroyOnLoad(go);
                }
                coroutineRunner = go.GetComponent<MonoBehaviour>();
                if (coroutineRunner == null)
                {
                    coroutineRunner = go.AddComponent<EmptyMonoBehaviour>();
                }
            }
        ...
        static bool Prefix(ref Vector3d universePosition, Quaternion r)
        {
            ...
            if (!flag2)
            {
                ...
                if (!flag3)
                {
                    ...
                    // If this is a multi-enemy spawn, use delays
                    if (enemies > 1)
                    {
                        float spawnDelay = ProfileManager.Instance.GetModule<AP_MConfig>().SpawnDelay;
                        GetCoroutineRunner().StartCoroutine(SpawnEnemiesWithDelay(
                            enemies,
                            isCustomEncounter,
                            encounter,
                            randomSelection,
                    ...
                    {
                        // Single enemy spawn or delay disabled
                        SpawnEnemies(enemies, isCustomEncounter, encounter, randomSelection,
                                   difficultyfactor, dropoff, ref faction, ref enemynames);
                    }
                }
            }
            return false;
        ...
        private static IEnumerator SpawnEnemiesWithDelay(
            int totalEnemies,
            bool isCustomEncounter,
            CustomEncounter encounter,
            RandomSelection<WorldSpecificationFactionDesign> randomSelection,
            float difficultyfactor,
            float dropoff,
            float delaySeconds,
            FactionObjectId faction,  
            List<string> enemynames)
        {
            for (int i = 0; i < totalEnemies; i++)
            {
                ...
                else
                {
                    ...
                    float encounterDifficultyFactor = currentDifficultyFactor;
                    ...
                    worldSpecificationFactionDesign2 = randomSelection.Select(delegate (WorldSpecificationFactionDesign t)
                    {
                        ...
                        float difficulty = InstanceSpecification.i.Adventure.WarpPlaneDifficulty;
                        difficulty = Math.Min(100, Math.Max(1, encounterDifficultyFactor * difficulty));
                        if (Math.Abs(difficulty - t.AdventureModeDifficultyMean) > 15f && encounter.MinDifficulty == 0)
                            return 0.0f;

                        AdvLogger.LogInfo("Added enemy to selection: " + t.Name + " difficulty: " +
                            t.AdventureModeDifficultyMean + " type: " + t.BlueprintType.ToString() +
                            " faction: " + t.CorporationID.ToString());

                        float num3 = Maths.NormalProbability(difficulty, t.AdventureModeDifficultyMean,
                            t.AdventureModeDifficultySigma) * t.AdventureModeChance;
                        return (double)num3;
                    });
                }
        ...
        private static void SpawnEnemies(
            ...
            float difficultyfactor,
            float dropoff,
            ref FactionObjectId faction,
            ref List<string> enemynames)
        {
            for (int i = 0; i < totalEnemies; i++)
            {
                WorldSpecificationFactionDesign worldSpecificationFactionDesign2 = null;
                ...
                else
                {
                    ...
                    worldSpecificationFactionDesign2 = randomSelection.Select(delegate (WorldSpecificationFactionDesign t)
                    {
                        ...
                        float difficulty = InstanceSpecification.i.Adventure.WarpPlaneDifficulty;
                        difficulty = Math.Min(100, Math.Max(1, currentDifficultyFactor * difficulty));
                        if (Math.Abs(difficulty - t.AdventureModeDifficultyMean) > 15f && encounter.MinDifficulty == 0)
                            return 0.0f;
